<template>
  <div class="create">
    ...
    <div class="create-c">
      <div class="cc-group">
        <div class="cc-gt font-z-16 font-c-666">{{$t('wallet.import.inputPrivate')}}</div>
        <div class="cc-input"><textarea name="" id="" rows="3" :placeholder="$t('wallet.import.privatePH')" v-model="param.private" @input="checkAddr"></textarea></div>
      </div>
      <div class="cc-group">
      ...
      <div class="btn-style font-z-16" v-if="isImport" :class="{'btn-style-click':isImport}" @click="importByPrivate"><span>{{$t('wallet.import.import')}}</span></div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    watch: {
      param: {
        deep: true,
        handler (nVal, oVal) {
          let len = this.param.psw.length >= 9;
          if (this.isRegister) {
            this.isImport = (this.param.walletName && this.param.private && len && this.param.psw && this.param.psw === this.param.rePsw)
          } else {
            this.isImport = (this.param.walletName && this.param.private && this.param.nickname && this.param.invite && len && this.param.psw && this.param.psw === this.param.rePsw)
          }
        }
      }
    },
    methods: {
      checkAddr() {
        let walletPrivate = this.param.private.trim();
        let addr = this.$sec.privateToAddress(Buffer.from(walletPrivate,'hex')).toString('hex');
        if (addr) {
          let checkRegister = {
            address: addr
          };
          ...
          this.$toast({
            message: this.$t('wallet.import.privateErr'),
            position: 'bottom',
            duration: 3000
          })
        }
      ...
      importByPrivate() {
        ...
        let walletPrivate = that.param.private.trim();
        ...
        setTimeout(()=> {
          ...
          if (!walletName) {
            ...
            let key = wal.makeWalletByPrivate(walletPrivate, that.param.psw);
            let eth = this.$sd_eth.fromPrivateForEth(that.$common.privateTest(walletPrivate));
            key.address = addr;
                ...
                if (data1.length) {
                  ...
                  for(let item of data1) {
                    if (item.address === addr) {
                      ...
                      break
                    } else if (item.name === that.param.walletName) {
                      that.$common.closeLoad();
                      this.$toast({
                        message: this.$t('wallet.exist'),
                        position: 'bottom',
